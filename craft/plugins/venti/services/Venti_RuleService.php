...
class Venti_RuleService extends BaseApplicationComponent
{
        ...
        */
        ...
        if ( $frequency != null)
        {
            ...
            if ($starts != null)
            {
                ...
                //$sdate = new \DateTime($stime,new \DateTimeZone(craft()->getTimezone()));
    ...
    */
    public function modalValuesArray($rule)
    {
        ...
        foreach ($keyValueAry as $key => $value)
        {
            switch ($key)
            {
                ...
                case 'DTEND':
                    $edate = new \DateTime($value, new \DateTimeZone(craft()->getTimeZone()));
                ...
                case 'UNTIL':
                    ...
                    $date = DateTime::createFromFormat("Ymd\THis\Z", $value, craft()->getTimeZone());
                ...
                case 'EXDATE':
                    ...
                    for ($i=0; $i < count($datesAry); $i++) {
                        $exdate = DateTime::createFromFormat("Ymd", $datesAry[$i], craft()->getTimeZone());
                        if ($exdate) {
                ...
                case 'RDATE':
                    ...
                    $indates = [];
                    for ($i=0; $i < count($datesAry); $i++) {
                        $indate = DateTime::createFromFormat("Ymd\THis\Z", $datesAry[$i], craft()->getTimeZone());
                        if ($indate) {
	...
	{
		return new DateTime($str,new \DateTimeZone(craft()->getTimeZone()));
	}
    ...
    */
    ...
    public function recurTextTransform($recurRule, $lang = null)
    {
        ...
        foreach (glob(craft()->path->getPluginsPath().'/venti/vendor/simshaun/recurr/translations/*.php') as $filepath)
        {
